...
void main()
{
    ...
    vec3 cam_axisx = cross(cam_axisy, cam_axisz);
    mat4 campos_xform = mat4(
        vec4(cam_axisx, 0.0),
        vec4(cam_axisy, 0.0),
        vec4(cam_axisz, 0.0),
        vec4(cam_trans, 1.0)
    );
    vsOut.campos = (campos_xform * glH_ObjectMatrix * vec4(P, 1.0)).xyz;
    vsOut.cam_zoom = cam_zoom;
    vsOut.cam_aspect = cam_aspect;
