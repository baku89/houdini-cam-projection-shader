...
void main()
{
    ...
    vec3 campos = fsIn.campos;

    vec2 texcoord_proj = vec2(
      .5 + fsIn.cam_zoom * -campos.x / campos.z,
      .5 + fsIn.cam_zoom * fsIn.cam_aspect * -campos.y / campos.z
    );

    // is inside of frustrum?
    float inside =
        // X-clipping
        step(0.0, texcoord_proj.x) * step(texcoord_proj.x, 1.0)
        // Y-clipping
        * step(0.0, texcoord_proj.y) * step(texcoord_proj.y, 1.0)
        // Z-clipping
        * step(campos.z, 0.0);

    // read in texture maps 
    if(has_diffuse_map) {
        tex = HOUsampleDiffuseMap(texcoord_proj);
        tex.rgb = mix(vec3(.5), tex.rgb, inside);
    } else {
        tex = vec4(1.0);
    }
